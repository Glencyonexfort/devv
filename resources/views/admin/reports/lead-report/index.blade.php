        ...
        <div class="card-body pb-0">
            ...
            <form action="" id="filter-form" style="width: 100%">
                ...
                <div class="row">
                    ...
                    <div class="col-md-4">
                        <div class="form-group">
                        <label class="txt14 w400 col-lg-12">@lang('app.report.source')</label>
                        <div>
                            <div class="col-lg-12 pull-left">
                                <select class="form-control" name="source">
                                    <option value="">@lang('app.report.noneSelected')</option>
                                    @foreach ($list_options as $source)
                                        <option value="{{ $source->options }}">{{ $source->options }}</option>
                                    @endforeach
                                </select>
                            </div>
                        </div>
...
<script>
    ...
    $(function() {
        ...
        $('#apply-filters').click(function(e) {                
                        e.preventDefault();
                        if($('#created_date_start').val() && $('#created_date_end').val())
                        {
                        var url = "{{ route('admin.crm-lead-get-data') }}";
                            $.ajax({
                                type: "GET",
                                url: url,
                                ...
                                success: function (result) {
                                    if(result.success == 1)
                                    {
                                        ...
                                        trHTML +=    '<tr>'+
                                                '<td>' + result.source + '</td>'+
                                                '<td>' + result.quotesCreated + '</td>'+
                                                '<td>' + result.jobsConfirmed + '</td>'+
                                                '<td>' + result.quotesLost + '</td>'+
