        ...
        <div style="flex:auto">
            <div class="card">
                <div class="card-header bg-white header-elements-inline">
                    <h6 class="card-title">@lang('app.menu.connectXero')</h6>
                    @if($tenant_api_details)
                    <a href="{{ route('admin.connect-xero.disconnect') }}" class="btn btn-outline-danger">Disconnect</a>
                    @endif
                </div>
                <div class="card-body">
                        <div class="row">
                            <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12" style="text-align: center">
                                <img src="{{asset('img/connectxero.jpg')}}" class="img img-responsive img-powered-by-stripe" alt="Connect XERO">
                            </div>
                            @if($tenant_api_details)
                                <?php
                            ...
                            <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">   
                                ...
                                <div class="form-group row">
                                    ...
                                    <select name="account_id" id="account_id" class="form-control col-lg-5">
                                        ...
                                        @foreach($accounts as $account)
                                            ...
                                            @if($account['Code']==$tenant_api_details->account_key)
                                            selected=""
                                            @endif
                                ...
                                <div class="form-group row">
                                    ...
                                    <select name="payment_account_id" id="payment_account_id" class="form-control col-lg-5">
                                        ...
                                        @if($account['Code']==960)@continue;@endif
                                            <option value="{{ $account['AccountID'] }}"
                                            @if($account['AccountID']==$tenant_api_details->smtp_secret)
                                            selected=""
                                            @endif
                                            >{{ $account['Code'].' - '.$account['Name'] }}</option>
                                        @endforeach
                            ...
                            <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
                                @if($myob_connected)
                                    <div class="alert alert-info">MYOB is already connected!</div>
                                @else
                                    <span data-xero-sso data-href="{{ route('admin.connect-xero.authorize') }}" data-label="Connect with Xero"></span>
...
<script>
    $('body').on('click', '#save_config', function(e) {
        ...
        $.ajax({
            url: "/admin/settings/connect-xero/storeConfig",
            method: 'POST',
            data: $("#xero_config").serialize(),
            dataType: "json",
            beforeSend: function() {
                $.blockUI();
            },
