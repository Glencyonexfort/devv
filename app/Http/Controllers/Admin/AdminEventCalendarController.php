...
class AdminEventCalendarController extends AdminBaseController
{
    ...
    public function store(StoreEvent $request)
    {
        ...
        if($request->user_id){
            ...
            $attendees = User::where(['tenant_id' => auth()->user()->tenant_id])->whereIn('id', $request->user_id)->get();
            Notification::send($attendees, new EventInvite($event));
        }
    ...
    public function update(UpdateEvent $request, $id)
    {
        ...
        if($request->all_employees){
            $attendees = User::allEmployees();
            foreach($attendees as $attendee){
                $checkExists = EventAttendee::where(['user_id' => $attendee->id, 'tenant_id' => auth()->user()->tenant_id])->where('event_id', $event->id)->first();
                if(!$checkExists){
                    EventAttendee::create(['user_id' => $attendee->id, 'event_id' => $event->id]);
        ...
        if($request->user_id){
            foreach($request->user_id as $userId){
                $checkExists = EventAttendee::where(['user_id' => $userId, 'tenant_id' => auth()->user()->tenant_id])->where('event_id', $event->id)->first();
                if(!$checkExists){
