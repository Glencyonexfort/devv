...
class UpdateWorksuiteVersionController extends Controller
{
    ...
    private function checkPermission()
    {
        ...
        if( config('froiden_envato.allow_users_id') !== null ){

            // 1
            if( config('froiden_envato.allow_users_id') === false ) return true;

            // 2
            if( in_array(Auth::User()->id, config('froiden_envato.allow_users_id')) === true ) return true;
        }
