...
if (rs_ary[3] != '' && rs_ary[3] != undefined) {
    var city_ary = rs_ary[3].split('=');
    var city_id = city_ary[1];
}

if (rs_ary[4] != '' && rs_ary[4] != undefined) {
    var discount_ary = rs_ary[4].split('=');
    var discount = discount_ary[1];
}else{
    var discount=0;
}

var domain_url = JSFileParentPath;
var live_url = domain_url + 'quote-lease-cleaning/' + tenant_id + '/' + company_id + '/' + city_id + '/' + discount;
document.write('<meta name="viewport" content="width=device-width, initial-scale=1.0"><style type="text/css">#calculatorBox{position:fixed;z-index:9999999999;right:0;top:6%;width:auto}#calculatorInnerBox{display:" + display_type + ";float:right;width:480px;height:600px;background:#fff;border:1px solid #f7f7f7;z-index:9999999999;padding:10px 15px;border-radius:15px 0px 0px 15px;-moz-border-radius:15px 0px 0px 15px;-ms-border-radius:15px 0px 0px 15px;-o-border-radius:15px 0px 0px 15px;-webkit-border-radius:15px 0px 0px 15px;color:#fff;-webkit-box-shadow:-3px 0px 15px -2px rgba(0, 0, 0, 0.28);-moz-box-shadow:-3px 0px 15px -2px rgba(0, 0, 0, 0.28);box-shadow:-3px 0px 15px -2px rgba(0,0,0,0.28)}#calculatorBtn{line-height:28px;float:right;-webkit-transform:rotate(90deg);-moz-transform:rotate(90deg);-ms-transform:rotate(90deg);-o-transform:rotate(90deg);transform:rotate(90deg);border-radius:0px 0px 15px 15px;-moz-border-radius:0px 0px 15px 15px;-ms-border-radius:0px 0px 15px 15px;-o-border-radius:0px 0px 15px 15px;-webkit-border-radius:0px 0px 15px 15px;padding:5px 15px;font-size:16px;color:#fff;margin-right:-65px;margin-top:220px;text-transform:uppercase;font-weight:bold;letter-spacing:1px;cursor:pointer;font-family:Arial,Helvetica,sans-serif;background:#f5446a}#calculatorInnerBox div{width:180px;color:#fff;font-size:18px;background:#3da35a;padding:3px 10px;margin-bottom:5px;text-align:left;border-radius:5px;-moz-border-radius:5px;-webkit-border-radius:5px;-o-border-radius:5px;-ms-border-radius:5px}#calculatorInnerBox input[type="submit"]{width:180px;color:#fff;font-size:18px;background:#3da35a;padding:3px 10px;margin-bottom:5px;text-align:center;border-radius:5px;-moz-border-radius:5px;-webkit-border-radius:5px;-o-border-radius:5px;-ms-border-radius:5px;margin-top:15px}#calculatorInnerBox div span{color:#fff;font-size:14px;border-radius:5px}#iframeBox{width:100%;height:100%;border:none}@media only screen and (max-width: 550px){#calculatorInnerBox{width:320px}}</style><div id="calculatorBox"><div id="calculatorInnerBox" style="display: ' + display_type + ';"><iframe id="iframeBox" src="' + live_url + '"></iframe></div><div id="calculatorBtn" onclick="showCalculatorBox(\'calculatorInnerBox\')"><img src="' + domain_url + 'quote-assets/quote.png" style="height: 13px; display: inline; width: auto;" /> GET A QUOTE</div></div>');

function showCalculatorBox(newId) {
    if (document.getElementById(newId).style.display == 'block') {
        ...
        document.getElementById('calculatorBtn').style.marginRight = '-65px';
    } else {
